<template>
  <v-card>
    <v-tabs>
      <v-tab to="/vulnerability-activity/list">리스트</v-tab>
      <!-- <v-tab to="/vulnerability-activity/graph">그래프</v-tab>
      <v-tab to="/vulnerability-activity/dashboard">대시보드</v-tab>
      <v-tab to="/vulnerability-activity/analyzer">분석기</v-tab> -->
    </v-tabs>
    <nuxt-child />
...
import VADetail from '~/components/VADetail.vue'
  ...
  methods: {
    ...
    submitItem(item) {
      ...
      if (id) {
        ...
        console.log(id)
        method = 'put'
        url = `http://localhost:8000/vulnerability-activity/${id}`
        ...
        method = 'post'
        url = `http://localhost:8000/vulnerability-activity`
    ...
    async loadItems() {
      this.items = []
      await axios
        .get('http://localhost:8000/vulnerability-activity/all', {
          headers: {
            'Access-Control-Allow-Origin': '*',
            'Access-Control-Allow-Methods':
              'GET, POST, PATCH, PUT, DELETE, OPTIONS',
    ...
    deleteItem() {
      ...
      console.log(id)
      const method = 'delete'
      const url = `http://localhost:8000/vulnerability-activity/${id}`
