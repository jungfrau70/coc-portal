<template>
  <v-card>
    <v-tabs>
      <v-tab to="/management-capacity/list">리스트</v-tab>
      <!-- <v-tab to="/capacity/graph">그래프</v-tab>
      <v-tab to="/capacity/dashboard">대시보드</v-tab>
      <v-tab to="/capacity/analyzer">분석기</v-tab> -->
    </v-tabs>
    <nuxt-child />
  ...
  methods: {
    ...
    submitItem(item) {
      ...
      if (id) {
        ...
        console.log(id)
        method = 'put'
        url = `http://localhost:8000/capacity/${id}`
        ...
        method = 'post'
        url = `http://localhost:8000/capacity`
    ...
    async loadItems() {
      this.items = []
      await axios
        .get('http://localhost:8000/capacity/all', {
          headers: {
            'Access-Control-Allow-Origin': '*',
            'Access-Control-Allow-Methods':
              'GET, POST, PATCH, PUT, DELETE, OPTIONS',
    ...
    deleteItem() {
      ...
      console.log(id)
      const method = 'delete'
      const url = `http://localhost:8000/capacity/${id}`
