    ...
    <v-form ref="form" v-model="valid" lazy-validation>
      <v-container>
        ...
        <v-row>
          ...
          <v-col>
            <v-text-field
              v-model="item['instance_ip']"
              label="IP"
            ></v-text-field>
          </v-col>
          <v-col>
            <v-text-field
              v-model="item['freq_full_archive']"
              label="Frequency"
            ></v-text-field>
          </v-col>
        </v-row>
...
export default {
  ...
  data: (vm) => ({
    ...
    colOptions: {
      ...
      progress: ['O', 'X'],
      status: ['None', 'O', 'X'],
      db_type: ['PostgreSQL', 'Maria', 'Tibero', 'Mongo', 'Redis'],
      freq_full_archive: ['Daily', 'Weekly', 'Monthly', 'Ad-hoc'],
    },
  }),
  computed: {
    selected: {
  ...
  methods: {
    ...
    setDefaultItem() {
      ...
      this.item.progress = this.item.progress || null
      this.item.status = this.item.status || null

      this.item.db_type = this.item.db_type || 'PostgreSQL'
      this.item.instance_name = null
      this.item.instance_ip = null
      this.item.freq_full_archive = this.item.freq_full_archive || 'Weekly'
