<template>
  <v-card>
    <v-tabs>
      ...
      <!-- <v-tab to="/database/graph">그래프</v-tab>
      <v-tab to="/database/dashboard">대시보드</v-tab>
      <v-tab to="/database/analyzer">분석기</v-tab> -->
    </v-tabs>
    <nuxt-child />
  ...
  methods: {
    ...
    submitItem(item) {
      ...
      if (id) {
        ...
        console.log(id)
        method = 'put'
        url = `http://localhost:8000/database/${id}`
    ...
    async loadItems() {
      this.items = []
      await axios
        .get('http://localhost:8000/database/all', {
          headers: {
            'Access-Control-Allow-Origin': '*',
            'Access-Control-Allow-Methods':
              'GET, POST, PATCH, PUT, DELETE, OPTIONS',
    ...
    deleteItem() {
      ...
      console.log(id)
      const method = 'delete'
      const url = `http://localhost:8000/database/${id}`
