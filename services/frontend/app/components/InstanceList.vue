<template>
  <v-card>
    <v-tabs>
      <v-tab to="/management-instance/list">리스트</v-tab>
      <!-- <v-tab to="/instance/graph">그래프</v-tab>
      <v-tab to="/instance/dashboard">대시보드</v-tab>
      <v-tab to="/instance/analyzer">분석기</v-tab> -->
    </v-tabs>
    <nuxt-child />
  ...
  methods: {
    ...
    submitItem(item) {
      ...
      if (id) {
        ...
        console.log(id)
        method = 'put'
        url = `http://localhost:8000/instance/${id}`
    ...
    async loadItems() {
      this.items = []
      await axios
        .get('http://localhost:8000/instance/all', {
          headers: {
            'Access-Control-Allow-Origin': '*',
            'Access-Control-Allow-Methods':
              'GET, POST, PATCH, PUT, DELETE, OPTIONS',
    ...
    deleteItem() {
      ...
      console.log(id)
      const method = 'delete'
      const url = `http://localhost:8000/instance/${id}`
