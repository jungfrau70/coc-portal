<template>
  <v-card>
    <v-tabs>
      <v-tab to="/management-backup/list">리스트</v-tab>
      <v-tab to="/management-backup/dashboard">대시보드</v-tab>
      <!-- <v-tab to="/management-backup/graph">그래프</v-tab>
      <v-tab to="/management-backup/analyzer">분석기</v-tab> -->
    </v-tabs>
    <nuxt-child />
...
const API_URL = `${process.env.BASE_URL}/backup`
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'DB Type', value: 'db_type', sortable: true },
        { text: 'Instance', value: 'instance_name', sortable: true },
        { text: 'IP', value: 'instance_ip', sortable: true },
  ...
  methods: {
    ...
    submitItem(item) {
      ...
      const data = {
        ...
        instance_ip: item.instance_ip,
        freq_full_archive: item.freq_full_archive,
