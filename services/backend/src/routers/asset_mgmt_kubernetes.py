...
from services.backend.src.cruds import _asset_mgmt_kubernetes
...
async def upload_file(file: Union[UploadFile, None] = None, db: Session = Depends(get_db)):
    if file.filename.lower().endswith(('.csv')):
        return _asset_mgmt_kubernetes.upload_csv(file, db)
    else:
...
def destroy(id:int, db: Session = Depends(get_db)):
    return _asset_mgmt_kubernetes.destroy(id,db)

@router.put('/{id}', status_code=status.HTTP_202_ACCEPTED)
# def update(id:int, request: schemas.Blog, db: Session = Depends(get_db),current_user: schemas.User = Depends(oauth2.get_current_user)):
def update(id:int, request: Schema, db: Session = Depends(get_db)):
    return _asset_mgmt_kubernetes.update(id,request, db)
