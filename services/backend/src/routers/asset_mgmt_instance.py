...
from services.backend.src.cruds import _asset_mgmt_instance

router = APIRouter(
    prefix="/instance",
    tags=['Asset Management ::: Instance']
)

get_db = database.get_db

Schema = schemas.ShowInstance

@router.get('/all', response_model=List[Schema])
# def all(db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
def all(db: Session = Depends(get_db)):
    return _asset_mgmt_instance.get_all(db)

@router.get('/{id}', status_code=200, response_model=Schema)
# def show(id:int, db: Session = Depends(get_db),current_user: schemas.User = Depends(oauth2.get_current_user)):
def show(id:int, db: Session = Depends(get_db)):
    return _asset_mgmt_instance.show(id,db)

@router.post('/', status_code=status.HTTP_201_CREATED,)
# def create(request: schemas.Blog, db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
def create(request: Schema, db: Session = Depends(get_db)):
    return _asset_mgmt_instance.create(request, db)
...
async def upload_file(file: Union[UploadFile, None] = None, db: Session = Depends(get_db)):
    if file.filename.lower().endswith(('.csv')):
        return _asset_mgmt_instance.upload_csv(file, db)
    else:
