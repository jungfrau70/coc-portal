...
def upload_csv(file, db: Session):
    ...
    df['api_cert_expired_date'] = df['api_cert_expired_date'].fillna(np.nan).replace([np.nan], ['1970-01-01'])
    df['ca_cert_expired_date'] = df['ca_cert_expired_date'].fillna(np.nan).replace([np.nan], ['1970-01-01'])
    df['etcd_cert_expired_date'] = df['etcd_cert_expired_date'].fillna(np.nan).replace([np.nan], ['1970-01-01'])
...
#     df['api_cert_expired_date'] = df['api_cert_expired_date'].fillna(np.na).replace([np.nan], ['1970-01-01'])
#     df['ca_cert_expired_date'] = df['ca_cert_expired_date'].fillna(np.nan).replace([np.nan], ['1970-01-01'])
#     df['etcd_cert_expired_date'] = df['etcd_cert_expired_date'].fillna(np.nan).replace([np.nan], ['1970-01-01'])
